...
class Scene(object):
    ...
    def __init__(
        ...
        self.main_loop_callbacks: list[Callable[[], None]] = []
    ...
    def _emit_frame_sinks(self) -> None:
        for frame_sink in list(self.frame_sinks):
            try:
                ...
                log.exception(
                    "Frame sink failed while consuming a frame"
                )
    ...
    def update_frame(
        self, dt: float = 0, force_draw: bool = False
    ) -> None:
        ...
        capture_swap = (
            not self.file_writer.write_to_movie
        ...
        if (
            self.window
            and not self.file_writer.write_to_movie
            and not capture_swap
        ):
            self.window.swap_buffers()
            if self.camera.fbo is not self.camera.window_fbo:
                self.camera.blit(
                    self.camera.fbo, self.camera.window_fbo
                )
                self.window.swap_buffers()
    ...
    def get_top_level_mobjects(self) -> list[Mobject]:
        ...
        def is_top_level(mobject):
            num_families = sum(
                [(mobject in family) for family in families]
            )
    ...
    def assemble_render_groups(self):
        ...
        batches = batch_by_property(
            ...
            lambda m: (
                str(type(m))
                + str(m.get_shader_wrapper(self.camera.ctx).get_id())
                + str(m.z_index)
            ),
        )
        ...
        for group in self.render_groups:
            group.clear()
        self.render_groups = sorted(
            [
                batch[0].get_group_class()(*batch)
                for batch, key in batches
            ],
            key=lambda m: getattr(m, "is_magnifying_glass", False),
        )

    @staticmethod
    def affects_mobject_list(
        func: Callable[..., T],
    ) -> Callable[..., T]:
        @wraps(func)
    ...
    def add(self, *new_mobjects: Mobject):
        ...
        id_to_scene_order = {
            id(m): idx for idx, m in enumerate(self.mobjects)
        }
        self.mobjects.sort(
            key=lambda m: (m.z_index, id_to_scene_order[id(m)])
        )
    ...
    def remove(self, *mobjects_to_remove: Mobject):
        ...
        """
        to_remove = set(
            extract_mobject_family_members(mobjects_to_remove)
        )
        new_mobjects, _ = recursive_mobject_remove(
    ...
    def get_time_progression(
        ...
        times = (
            np.arange(0, run_time, 1 / self.camera.fps)
            + 1 / self.camera.fps
        )

        self.file_writer.set_progress_display_description(
            sub_desc=desc
        )
    ...
    def get_run_time(self, animations: Iterable[Animation]) -> float:
        return np.max(
            [animation.get_run_time() for animation in animations]
        )
    ...
    def start_mic_recording(
        ...
        channels: int = 1,
        chunk: int = 1024,
        callback: Callable[[bytes, int, dict, int], None]
        | None = None,
    ) -> None:
        self.file_writer.start_mic_recording(
            rate, channels, chunk, callback
        )
    ...
    def add_sound(
        ...
        self.file_writer.add_sound(
            sound_file, time, gain, gain_to_background
        )
    ...
    def save_state(self) -> None:
        ...
        if self.undo_stack and state.mobjects_match(
            self.undo_stack[-1]
        ):
            return
        self.redo_stack = []
    ...
    def is_window_closing(self):
        return self.window and (
            self.window.is_closing or self.quit_interaction
        )
    ...
    def set_floor_plane(self, plane: str = "xy"):
        if plane == "xy":
            ...
            raise Exception(
                "Only `xz` and `xy` are valid floor planes"
            )
    ...
    def on_mouse_motion(self, point: Vect3, d_point: Vect3) -> None:
        ...
        if self.is_key_pressed(ord(manim_config.key_bindings.pan_3d)):
            ff_d_point = frame.to_fixed_frame_point(
                d_point, relative=True
            )
    ...
    def on_mouse_press(
        self, point: Vect3, button: int, mods: int
    ) -> None:
        self.mouse_drag_point.move_to(point)
    ...
    def on_key_press(self, symbol: int, modifiers: int) -> None:
        ...
        if char == manim_config.key_bindings.reset:
            ...
            modifiers
            & (
                PygletWindowKeys.MOD_COMMAND
                | PygletWindowKeys.MOD_CTRL
            )
        ):
            self.redo()
        elif char == "z" and (
            ...
            self.undo()
        # command + q
        elif char == manim_config.key_bindings.quit and (
            modifiers
            & (
                PygletWindowKeys.MOD_COMMAND
                | PygletWindowKeys.MOD_CTRL
...
class SceneState:
    def __init__(
        ...
        dont_modify: list[Mobject] | None = None,
    ):
        ...
        last_m2c = (
            scene.undo_stack[-1].mobjects_to_copies
            if scene.undo_stack
            else dict()
        )
    ...
    def restore_scene(self, scene: Scene):
        ...
        scene.mobjects = [
            ...
            if mob not in self.dont_modify
            else mob
...
class ThreeDScene(Scene):
    ...
    def add(
        self,
        *mobjects: Mobject,
        set_depth_test: bool = True,
        perp_stroke: bool = True,
    ):
        for mob in mobjects:
            if (
                set_depth_test
