...
class SVGMobject(VMobject):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if svg_string != "":
            ...
            self.svg_string = self.file_name_to_svg_string(
                self.file_name
            )
        else:
            raise Exception(
    ...
    def _rects_overlap(
        ...
        bx0, by0, bx1, by1 = b
        return not (
            (ax1 < bx0) or (ax0 > bx1) or (ay1 < by0) or (ay0 > by1)
        )
    ...
    def _apply_overlap_safe_batching(
        self, mobs: list[VMobject]
    ) -> None:
        ...
        for mob in mobs:
            ...
            stroke_w = (
                float(data["stroke_width"][0, 0])
                if "stroke_width" in data.dtype.names
                else 0.0
            )
        ...
        for mob, rect in zip(mobs, rects):
            placed = False
            ix0, ix1, iy0, iy1 = self._rect_to_cell_range(
                rect, cell_size
            )
            ...
            for gid, group in enumerate(groups):
                ...
                if any(
                    self._rects_overlap(rect, group_rects[i])
                    for i in candidates
                ):  # type: ignore[index]
                    continue
    ...
    def _quantize_rgba(rgba: np.ndarray) -> tuple[int, int, int, int]:
        ...
        vals = [
            int(np.clip(round(255 * float(x)), 0, 255)) for x in rgba
        ]
    ...
    def _style_signature(cls, mob: VMobject) -> tuple:
        ...
        stroke_width = int(
            round(1000 * float(data["stroke_width"][0, 0]))
        )
        fill_border_width = int(
            round(1000 * float(data["fill_border_width"][0, 0]))
        )
        stroke_behind = int(getattr(mob, "stroke_behind", False))
        flat_stroke = int(
            getattr(mob, "get_flat_stroke", lambda: False)()
        )
        joint_type = int(
            round(float(mob.uniforms.get("joint_type", 0.0)))
        )
        anti_alias_width = int(
            round(
                1000
                * float(mob.uniforms.get("anti_alias_width", 1.5))
            )
        )
        shading = tuple(
            int(round(1000 * float(v))) for v in mob.get_shading()
        )
        return (
            fill,
            stroke,
    ...
    def modify_xml_tree(
        self, element_tree: ET.ElementTree
    ) -> ET.ElementTree:
    ...
    def mobjects_from_svg(self, svg: se.SVG) -> list[VMobject]:
        ...
        for shape in svg.elements():
            ...
            #     mob = self.text_to_mobject(shape)
                ...
                if mob is None:
                    log.warning(
                        "Only SVG images with embedded base64 data are supported. Skipping image element."
                    )
                    continue
                ...
                log.warning(
                    "Unsupported element type: %s", type(shape)
                )
                continue
            if not mob.has_points() and not isinstance(
                mob, SVGMobject
            ):
                continue
            if isinstance(shape, se.GraphicObject) and not isinstance(
                mob, SVGMobject
            ):
    ...
    def apply_style_to_mobject(
        mob: VMobject, shape: se.GraphicObject
    ) -> VMobject:
        mob.set_style(
            stroke_width=0
            if not shape.stroke.hexrgb
            else shape.stroke_width,
            stroke_color=shape.stroke.hexrgb
            or (shape.fill.hexrgb or BLACK),
            stroke_opacity=shape.stroke.opacity or 1.0,
            fill_color=shape.fill.hexrgb or BLACK,
            fill_opacity=0
            if not shape.fill.hexrgb
            else shape.fill.opacity,
        )
        return mob

    def path_to_mobject(
        self, path: se.Path, svg: se.SVG
    ) -> VMobjectFromSVGPath:
        if path.id in svg.objects:
            ...
            mob = VMobjectFromSVGPath(
                ref_path, **self.path_string_config
            )
            ...
            return VMobjectFromSVGPath(
                path, **self.path_string_config
            )
    ...
    def ellipse_to_mobject(
        self, ellipse: se.Circle | se.Ellipse
    ) -> Circle:
    ...
    def image_to_mobject(self, image: se.Image) -> SVGMobject | None:
        ...
        mob.shift(
            (image.x or 0) * RIGHT + (image.width or 0) * RIGHT / 2
        )
...
class VMobjectFromSVGPath(VMobject):
    def __init__(self, path_obj: se.Path, **kwargs):
        ...
        self.transform_cache: (
            tuple[se.Matrix, np.ndarray, np.ndarray] | None
        ) = None
    ...
    def _point_in_polygon(
        point: np.ndarray, poly: np.ndarray
    ) -> bool:
    ...
    @classmethod
    def _find_point_inside_polygon(
        cls, poly: np.ndarray
    ) -> np.ndarray:
    ...
    def _normalize_subpath_orientations(self) -> None:
        ...
        for sp in subpaths:
            ...
            if len(poly) >= 2 and np.allclose(
                poly[0], poly[-1], atol=1e-6
            ):
                poly = poly[:-1]
            if len(poly) < 3:
                areas.append(0.0)
    ...
    def handle_commands(self) -> None:
        segment_class_to_func_map = {
            ...
            se.Line: (
                lambda p: self.add_line_to(p, allow_null_line=False),
                ("end",),
            ),
            se.QuadraticBezier: (
                lambda c, e: self.add_quadratic_bezier_curve_to(
                    c, e, allow_null_curve=False
                ),
                ("control", "end"),
            ),
            se.CubicBezier: (
        ...
        for segment in self.path_obj:
            ...
            if segment_class is se.Arc:
                ...
                func, attr_names = segment_class_to_func_map[
                    segment_class
                ]
                points = [
                    _convert_point_to_3d(
                        *segment.__getattribute__(attr_name)
                    )
        ...
        if self.has_new_path_started() and self.get_num_points() >= 2:
    ...
    def handle_arc(self, arc: se.Arc) -> None:
        if self.transform_cache is not None:
            ...
            transform = se.Matrix(
                self.path_obj.values.get("transform", "")
            )
            rot = np.array(
                [
        ...
        arc_points[:, 1] *= arc.ry
        arc_points @= np.array(
            rotation_about_z(arc.get_rotation().as_radians)
        ).T
