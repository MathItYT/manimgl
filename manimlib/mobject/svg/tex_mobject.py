    ...
    def make_number_changeable(
        ...
        **config,
        ...
        if index > len(parts) - 1:
            log.warning(f"Requested {index}th occurrence of {value}, but only {len(parts)} exist")
            return VMobject()
        ...
        # 1. Movido fuera del bucle: no hay necesidad de reevaluar esto por cada parte
        if "num_decimal_places" not in config:
            config["num_decimal_places"] = len(substr.split(".")[1]) if "." in substr else 0
        ...
        # 2. Lógica de reemplazo de string e indexado corregida
        if replace_all:
            self.string = self.string.replace(substr, R"\decimalmob")
            return VGroup(*decimal_mobs)
        else:
            # Reemplazar exactamente la n-ésima ocurrencia en el string interno
            string_parts = self.string.split(substr)
            if index < len(string_parts) - 1:
                before = substr.join(string_parts[:index + 1])
                after = substr.join(string_parts[index + 1:])
                self.string = " ".join([before, R"\decimalmob", after])
            
            # 3. Retorna el índice 0 porque 'parts' ya fue filtrado a un único elemento arriba
            return decimal_mobs[0]
